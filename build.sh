...
MAKEFILE='build-capkopper-corporate.make'
...
if [ -e "$TARGET/profiles/capkopper_corporate/modules/capkopper/capkopper_core" ]; then
  ...
  if [ -e "$DRUPAL/profiles/capkopper_corporate/local" ]; then
    echo "Remove the local folder that was used for the build (@see Make Local plugin for Drush)."
    rm -rf "$DRUPAL/profiles/capkopper_corporate/local"
  fi
