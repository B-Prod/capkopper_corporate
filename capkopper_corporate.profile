<?php
/**
 * @file capkopper_corporate.profile
 */
 ...
 * password is set).
 */
function capkopper_corporate_install_tasks($install_state) {
  ...
  $tasks['capkopper_corporate_select_features_form'] = array(
    'display_name' => st('Select features'),
    'type' => 'form',
  );

  $tasks['capkopper_corporate_additional_modules'] = array(
  ...
  $tasks['capkopper_corporate_finalize'] = array(
    'display_name' => st('Finalize installation'),
    'type' => 'batch',
    'run' => INSTALL_TASK_RUN_IF_REACHED,
    'display' => strpos($current_task, 'capkopper_corporate_finalize') === 0,
  );
 ...
 */
function capkopper_corporate_install_tasks_alter(&$tasks, $install_state) {
 ...
 */
function capkopper_corporate_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function capkopper_corporate_select_features_form() {
  ...
  foreach (capkopper_corporate_get_features() as $module => $info) {
 ...
 */
function capkopper_corporate_select_features_form_submit(&$form, &$form_state) {
 ...
 */
function capkopper_corporate_additional_modules() {
  $modules = array();
  $features = capkopper_corporate_get_features();
  ...
  foreach ($modules as $module) {
    if ($files[$module]->requires) {
      ...
      $dependencies = array_filter($dependencies, '_capkopper_corporate_filter_dependencies');
  ...
  foreach ($modules_sorted as $module => $weight) {
    $operations[] = array('_capkopper_corporate_enable_module', array($module, $files[$module]->info['name']));
  }

  $operations[] = array('_capkopper_corporate_flush_caches', array(st('Flushed caches.')));

  $batch = array(
    'title' => st('Installing extra features'),
    'operations' => $operations,
    'file' => drupal_get_path('profile', 'capkopper_corporate') . '/capkopper_corporate.install_callbacks.inc',
  );
 ...
 */
function capkopper_corporate_finalize() {
  $operations = array(
    array('_capkopper_corporate_clean_variables', array(st('Cleaning variables.')))
  );
 ...
 */
function capkopper_corporate_get_features() {
  return array(
 ...
 */
function _capkopper_corporate_filter_dependencies($dependency) {
